...
### Рекомендуемый способ: Использовать готовый SQL файл

```bash
# Применить тестовые данные (с автоматической очисткой старых данных)
psql -U asattorov -d energy_audit -f database/002_seed_test_data.sql
```

**✅ Что делает этот файл:**
- Автоматически удаляет старые тестовые данные (@example.com)
- Вставляет пользователей с **правильными** хешами паролей
- Использует подзапросы для корректных foreign key связей
- Все данные вставляются в одной транзакции

### Альтернативный способ: Сгенерировать только пользователей

Если нужны только пользователи без остальных данных:

```bash
cd database
python3 generate_test_users.py | psql -U asattorov -d energy_audit
```
